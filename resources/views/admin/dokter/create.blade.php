    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <form action="{{ route('admin.dokter.store') }}" method="POST" class="space-y-5">
            ...
            <div class="flex gap-3">
                <button type="submit"
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-bold shadow-sm">
                    Simpan
                </button>
                <a href="{{ route('admin.dokter.index') }}"
                    class="px-4 py-2 bg-slate-200 rounded-lg hover:bg-slate-300 transition text-slate-700 font-bold">
                    Batal
                </a>
            </div>
        </form>
